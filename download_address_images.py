"""
Script to download property images for address-based requests
"""
...
def download_images_for_address_requests():
    """Download property images for address-based requests"""
    ...
    images_dir = "address_based_images"
    ...
    try:
        ...
        print("Property data file not found. Run the address requests script first.")
        return
    ...
    # Download each image
    downloaded_count = 0
    for i, image_ref in enumerate(image_references[:3]):  # Limit to first 3 for demo
        if image_ref in imagery_data:
            image_info = imagery_data[image_ref]
            image_token = image_info.get('image_token')
            
            if image_token:
                print(f"\nDownloading image {i+1}/{min(3, len(image_references))}: {image_ref}")
...
if __name__ == "__main__":
    download_images_for_address_requests()